...
HT_st7735::HT_st7735(	int8_t cs_pin,int8_t rest_pin, int8_t  dc_pin,    
...
void HT_st7735::st7735_select(void) 
{
  digitalWrite(_cs_pin, LOW);
}

void HT_st7735::st7735_unselect(void)
{
  digitalWrite(_cs_pin, HIGH);
}

void HT_st7735::st7735_reset(void) 
{
...
void HT_st7735::st7735_write_cmd(uint8_t cmd)
{
...
void HT_st7735::st7735_set_address_window(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1) 
{
    // column address set
...
void HT_st7735::st7735_init(void) 
{
    ...
    st7735_spi.begin(_sclk_pin,-1,_mosi_pin);
    st7735_select();
    st7735_reset();
...
void HT_st7735::st7735_write_char(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor) 
{
    uint32_t i, b, j;
...
void  HT_st7735::st7735_write_str(uint16_t x, uint16_t y, String str_data, FontDef font, uint16_t color, uint16_t bgcolor) 
{
    const char *str=str_data.c_str();
    st7735_write_str( x, y, str,  font,  color,  bgcolor);
}

void  HT_st7735::st7735_write_str2(uint16_t x, uint16_t y, String str_data, FontDef font, uint16_t color, uint16_t bgcolor) 
{
    const char *str=str_data.c_str();
    st7735_write_str2( x, y, str,  font,  color,  bgcolor);
}

void  HT_st7735::st7735_write_str(uint16_t x, uint16_t y, const char *str, FontDef font, uint16_t color, uint16_t bgcolor) 
{
    st7735_select();
    while(*str) {
...
void  HT_st7735::st7735_write_str2(uint16_t x, uint16_t y, const char *str, FontDef font, uint16_t color, uint16_t bgcolor) 
{
    st7735_select();
    while(*str) {
        if(x + font.width >= _width) {
        ...
        st7735_write_char2(x, y, *str, font, color, bgcolor);
        x += font.width*2;
        str++;
    }
...
void HT_st7735::st7735_fill_rectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    // clipping
    ...
    for(y = h; y > 0; y--) 
    {
        for(x = w; x > 0; x--) 
        {
            st7735_spi.transfer(data, sizeof(data));
        }
    }
...
void HT_st7735::st7735_fill_screen(uint16_t color) 
{
    st7735_fill_rectangle(0, 0, _width, _height, color);
}


void HT_st7735::st7735_draw_image(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data) 
{
    if((x >= _width) || (y >= _height)) return;
...
void HT_st7735::st7735_write_char2(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor) 
{
    uint32_t i, b, j;

    // Увеличиваем размер окна в 2 раза
    st7735_set_address_window(x, y, x + font.width * 2 - 1, y + font.height * 2 - 1);

    for(i = 0; i < font.height; i++) {
        b = font.data[(ch - 32) * font.height + i];
        // Каждую строку рисуем дважды (по вертикали)
        for(uint8_t v_dup = 0; v_dup < 2; v_dup++) {
            for(j = 0; j < font.width; j++) {
                // Каждый пиксель рисуем дважды (по горизонтали)
                for(uint8_t h_dup = 0; h_dup < 2; h_dup++) {
                    if((b << j) & 0x8000) {
                        uint8_t data[] = { color >> 8, color & 0xFF };
                        st7735_write_data(data, sizeof(data));
                    } else {
...
void HT_st7735::st7735_invert_colors(bool invert) 
{
    st7735_select();
...
void HT_st7735::st7735_set_gamma(GammaDef gamma)
{
  uint8_t data[1];
